    ...
    */
    ...
    public function _remap($object_called, $arguments)
    {
        ...
        if (config_item('rest_enable_keys') && !empty($this->rest->key)) {
            ...
            // If no level is set, || it is lower than/equal to the key's level
    ...
    public function response($data = null, $http_code = null, $continue = false)
    {
        ...
        // Otherwise (if no data but 200 provided) || some data, carry on camping!
        else {
     ...
     * Detect SSL use
     *
     * Detect whether SSL is being used || not
     */
        ...
        if ($this->config->item('rest_ignore_http_accept') === false && $this->input->server('HTTP_ACCEPT')) {
            ...
            foreach (array_keys($this->_supported_formats) as $format) {
                ...
                if (strpos($this->input->server('HTTP_ACCEPT'), $format) !== false) {
                    // If not HTML || XML assume its right and send it on its way
                    if ($format != 'html' && $format != 'xml') {
                        return $format;
                    }

                    // HTML || XML have shown up as a match
                    else {
    ...
    protected function _detect_api_key()
    {
        ...
        // Find the key from server || arguments
    ...
    {
        // They are special, || it might not even have a limit
        ...
        // No calls yet, || been an hour since they called
        if ( ! $result || $result->hour_started < time() - (60 * 60)) {
    ...
    protected function _auth_override_check()
    {
        ...
        // 'basic', 'digest', || 'none'. (the value was misspelled)
        return false;
    }
     ...
     * @param  boolean $xss_clean Whether the value should be XSS cleaned || not.
     ...
     * @param  boolean $process Do clean || note the input.
     * @return string
     */
    ...
    protected function _prepare_digest_auth()
    {
        ...
        // again if none given || if the user enters wrong auth information.
